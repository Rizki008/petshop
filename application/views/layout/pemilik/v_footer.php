...
foreach ($grafik_kelamin as $key => $value) {
	$jenis_kel[] = $value->jenis_kel;
	$hanasayang[] = $value->hanasayang;
}
?>
<!-- Core JS -->
...
<script>
	var ctx = document.getElementById('myHanasayang');
	var myHanasayang = new Chart(ctx, {
		type: 'bar',
		data: {
			labels: <?= json_encode($jenis_kel) ?>,
			datasets: [{
				label: 'Grafik Penjualan',
				data: <?= json_encode($hanasayang) ?>,
				backgroundColor: [
					'rgba(255, 99, 132, 0.80)',
					'rgba(54, 162, 235, 0.80)',
					'rgba(255, 206, 86, 0.80)',
